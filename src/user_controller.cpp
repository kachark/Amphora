...
bool UserController::LoadUserList() {
    ...
    if (mediator_m.FindFile(filename)) {  // check if file exists in directory
        std::cout << "File found" << std::endl; // debug
        loadstatus = mediator_m.Load<User>(filename, uservector);
...
bool UserController::SaveUserList() {
    ...
    bool filefound = mediator_m.FindFile(filename);
    if (!filefound) {
    ...
    if (filefound) {
        std::cout << "File found" << std::endl; // debug
        savestatus = mediator_m.Save<User>(filename, uservector);
