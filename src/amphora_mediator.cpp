...
bool AmphoraMediator::FindFile(const std::string &filename) {
    return amphora_util_m->FindFile(filename);
}

template<typename T>
bool AmphoraMediator::Load(const std::string &filename, std::vector<T> &buffer) {
    return amphora_util_m->LoadFromFile(filename, buffer);
}

template<typename T>
bool AmphoraMediator::Save(const std::string &filename, std::vector<T> &buffer) {
